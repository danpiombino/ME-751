...
for nt = 2:T/h+1
    ...
    while n == 1
        ...
        %Kinematic Constraints
        [phi(1,k),nu(1,1),gamma(1,1),phi_r(1,:),phi_p(1,:),rf_q(:,:,1)] = phi_dp1(q(:,nt),q_d(:,nt),i,j,az_ib,ay_jb,th2,lam(1));
        [phi(2,k),nu(2,1),gamma(2,1),phi_r(2,:),phi_p(2,:),rf_q(:,:,2)] = phi_cd(q(:,nt),q_d(:,nt),i,j,sq_ib,so_jb,c1,dis,lam(2)); %fixed in x
        [phi(3,k),nu(3,1),gamma(3,1),phi_r(3,:),phi_p(3,:),rf_q(:,:,3)] = phi_cd(q(:,nt),q_d(:,nt),i,j,sq_ib,so_jb,c2,dis,lam(3)); %fixed in y
        [phi(4,k),nu(4,1),gamma(4,1),phi_r(4,:),phi_p(4,:),rf_q(:,:,4)] = phi_cd(q(:,nt),q_d(:,nt),i,j,sq_ib,so_jb,c3,dis,lam(4)); %fixed in z
        [phi(5,k),nu(5,1),gamma(5,1),phi_r(5,:),phi_p(5,:),rf_q(:,:,5)] = phi_dp1(q(:,nt),q_d(:,nt),i,j,az_ib,az_jb,th2,lam(5));
        %Driving Constraints
        if abs(theta(1,nt))*180/pi >= 10 %Change reference angle to avoid singularities
            [phi(6,k),nu(6,1),gamma(6,1),phi_r(6,:),phi_p(6,:),rf_q(:,:,6)] = phi_dp1(q(:,nt),q_d(:,nt),i,j,ax_ib,ay_jb,theta(:,nt),lam(6));
        else
            thetadev = theta(:,nt);
            thetadev(1) = theta(1,nt)+pi/4;
            [phi(6,k),nu(6,1),gamma(6,1),phi_r(6,:),phi_p(6,:),rf_q(:,:,6)] = phi_dp1(q(:,nt),q_d(:,nt),i,j,ax_ib,ay_jb,thetadev,lam(6));
        end
        %Euler Parameter Constraints
        [phi(7,k),nu(7,1),gamma(7,1),phi_r(7,:),phi_p(7,:),rf_p] = phi_ep(q(:,nt),q_d(:,nt),i,lam(7));
        phi_q = [phi_r phi_p]; %Combined Jacobian
        ...
        if method == 1
            ...
            tau_b_p = 8*G1_d'*J1_b*G1_d;
            tau_b_pd = 8*tmat(J1_b*G1_d*q([4*(i-1)+3*nb+1:4*(i-1)+3*nb+4],nt));
            
            G1_dd = gmat(q_dd(:,nt),i);
            Jp_p = -4*G1'*J1_b*G1_dd+4*tmat(J1_b*G1*q_dd([4*(i-1)+3*nb+1:4*(i-1)+3*nb+4],nt));
            ...
            psi = nrpsi(M,h,beta,J1p,Jp_p,phi_q,F_r,F_rd,F_p,F_pd,tau_b_r,tau_b_rd,tau_b_p,tau_b_pd,rf_q_full,rf_p);
        elseif method == 2
            if k == 1
                            %Full NR info
            F_r = zeros(3*nb,3*nb);
...
trun = toc
save(['A9P1_Method' num2str(method)],'t','h','trun','iter','Op_dd');

...
%         end
        ...
        %Kinematic Constraints
        [phi(1,k),nu(1,1),gamma(1,1),phi_r(1,:),phi_p(1,:),rf_q(:,:,1)] = phi_cd(q(:,nt),q_d(:,nt),1,0,sq1_1b,sq1_0b,ax,dis,lam(1));
        [phi(2,k),nu(2,1),gamma(2,1),phi_r(2,:),phi_p(2,:),rf_q(:,:,2)] = phi_cd(q(:,nt),q_d(:,nt),1,0,sq1_1b,sq1_0b,ay,dis,lam(2));
        [phi(3,k),nu(3,1),gamma(3,1),phi_r(3,:),phi_p(3,:),rf_q(:,:,3)] = phi_cd(q(:,nt),q_d(:,nt),1,0,sq1_1b,sq1_0b,az,dis,lam(3));
        [phi(4,k),nu(4,1),gamma(4,1),phi_r(4,:),phi_p(4,:),rf_q(:,:,4)] = phi_cd(q(:,nt),q_d(:,nt),2,1,sq2_2b,sq2_1b,ax,dis,lam(4));
        [phi(5,k),nu(5,1),gamma(5,1),phi_r(5,:),phi_p(5,:),rf_q(:,:,5)] = phi_cd(q(:,nt),q_d(:,nt),2,1,sq2_2b,sq2_1b,ay,dis,lam(5));
        [phi(6,k),nu(6,1),gamma(6,1),phi_r(6,:),phi_p(6,:),rf_q(:,:,6)] = phi_cd(q(:,nt),q_d(:,nt),2,1,sq2_2b,sq2_1b,az,dis,lam(6));
        [phi(7,k),nu(7,1),gamma(7,1),phi_r(7,:),phi_p(7,:),rf_q(:,:,7)] = phi_dp1(q(:,nt),q_d(:,nt),1,0,az,az,th,lam(7));
        [phi(8,k),nu(8,1),gamma(8,1),phi_r(8,:),phi_p(8,:),rf_q(:,:,8)] = phi_dp1(q(:,nt),q_d(:,nt),1,0,az,ay,th,lam(8));
        [phi(9,k),nu(9,1),gamma(9,1),phi_r(9,:),phi_p(9,:),rf_q(:,:,9)] = phi_dp1(q(:,nt),q_d(:,nt),2,0,az,az,th,lam(9));
        [phi(10,k),nu(10,1),gamma(10,1),phi_r(10,:),phi_p(10,:),rf_q(:,:,10)] = phi_dp1(q(:,nt),q_d(:,nt),2,0,az,ay,th,lam(10));
        %EP Constraints
        for i = 1:nb
            [phi(nc+i,k),nu(nc+i,1),gamma(nc+i,1),phi_r(nc+i,:),phi_p(nc+i,:),rf_p(:,:,i)] = phi_ep(q(:,nt),q_d(:,nt),i,lam(nc+i));
        end
        phi_q = [phi_r phi_p]; %Combined Jacobian
        ...
        if method == 1
            %Full NR info
            F_r = zeros(3*nb,3*nb);
            F_rd = zeros(3*nb,3*nb);
            F_p = zeros(3*nb,4*nb);
            F_pd = zeros(3*nb,4*nb);
            tau_b_r = zeros(4*nb,3*nb);
            tau_b_rd = zeros(4*nb,3*nb);
            tau_b_p = zeros(4*nb,4*nb);
            tau_b_p(1:4,1:4) = 8*G1_d'*J1_b*G1_d;
            tau_b_p(5:8,5:8) = 8*G2_d'*J2_b*G2_d;
            tau_b_pd = zeros(4*nb,4*nb);
            tau_b_pd(1:4,1:4) = 8*tmat(J1_b*G1_d*q([4*(1-1)+3*nb+1:4*(1-1)+3*nb+4],nt));
            tau_b_pd(5:8,5:8) = 8*tmat(J2_b*G2_d*q([4*(2-1)+3*nb+1:4*(2-1)+3*nb+4],nt));
            
            G1_dd = gmat(q_dd(:,nt),1);
            G2_dd = gmat(q_dd(:,nt),2);
            Jp_p = zeros(4*nb,4*nb);
            Jp_p(1:4,1:4) = -4*G1'*J1_b*G1_dd+4*tmat(J1_b*G1*q_dd([4*(1-1)+3*nb+1:4*(1-1)+3*nb+4],nt));
            Jp_p(5:8,5:8) = -4*G2'*J2_b*G2_dd+4*tmat(J2_b*G2*q_dd([4*(2-1)+3*nb+1:4*(2-1)+3*nb+4],nt));
            
            rf_q_full = zeros(size(rf_q,1),size(rf_q,2));
            for jn = 1:nc
                rf_q_full = rf_q_full + rf_q(:,:,nc);
            end
            rf_p_full = zeros(size(rf_p,1),size(rf_p,2));
            for jn = 1:nb
        ...
        if abs(norm(dz)) >= 1e-3 %are any phi values still not zero?
            dum1 = 0; %if so, do not stop while loop
        end
...
trun = toc
save(['A9P2_Method' num2str(method)],'t','h','trun','iter','O2_dd');
